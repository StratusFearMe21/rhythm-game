...
First, find a song that you think would be fitting and insert it into `src/songs/{song-name-kebab-case}/{song-name-kebab-case}.wav`. Second, alongside the `.wav` put in a `.rhythm` file with the same name as the `.wav` (leave it empty for now).
...
5. Run `bun run round src/songs/{your-song}/{your-song}.rhythm {BPM}` to round all of the notes to the nearest 32nd note. To figure out the BPM, I just used a tempo tapper.
